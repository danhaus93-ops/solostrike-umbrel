...
import { LS_PORTS, APP_VERSION } from '../App';
  ...
  return (
    <div className="ssdesk" ref={fitRef}
      ...
      <footer className="ss-foot">
        <span className="ff-brand">LoneStrike v{APP_VERSION} — ckpool-solo{poolState?.privateMode?' · 🔒 PRIVATE':''}</span>
        ...
        <span className="ff-r">Ports
          {[[LS_PORTS.main,LS_PORTS.main],[LS_PORTS.hobby,LS_PORTS.hobby],[LS_PORTS.tls,LS_PORTS.tls],...(LS_PORTS.sv2Pubkey?[[LS_PORTS.sv2,LS_PORTS.sv2]]:[])].map(([p,lbl],i)=>{const st=H.ports&&H.ports[p]&&H.ports[p].status;const c=st==='healthy'?'var(--green)':st==='degraded'?'var(--amber)':st==='down'?'var(--red)':'var(--cyan)';const glow=st==='healthy'||st==='degraded'||st==='down';return <React.Fragment key={p}>{i>0&&(i===2?<span className="tls">{_tt('TLS')}</span>:i===3?<span className="tls" style={{color:'var(--amber)',borderColor:'rgba(245,166,35,0.45)'}}>{_tt('SV2')}</span>:<span style={{opacity:.5}}> · </span>)}<b className="port" style={{color:c,textShadow:glow?`0 0 6px ${c}`:'none'}} onClick={()=>{try{navigator.clipboard.writeText(p===LS_PORTS.sv2?`stratum2+tcp://${window.location.hostname}:${p}/${LS_PORTS.sv2Pubkey}`:`stratum+${p===LS_PORTS.tls?'ssl':'tcp'}://${window.location.hostname}:${p}`);}catch(e){}}} title={st?`Port ${p} — ${st}`:`Port ${p} — checking…`}>{lbl}</b></React.Fragment>;})}
        </span>
      </footer>
