...
// Wraps each card with HTML5 drag+drop AND touch handlers.
// On mobile: long-press (600ms) on the ≡ handle to initiate drag.
  ...
  return (
    <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 1rem', height:52, borderBottom:'1px solid var(--border)', background:'rgba(6,7,8,0.95)', backdropFilter:'blur(8px)', position:'sticky', top:0, zIndex:50, gap:'0.5rem' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'0.6rem', fontFamily:'var(--fd)', fontSize:'0.58rem', color:'var(--text-2)', letterSpacing:'0.08em' }}>
        ...
        <button onClick={onSettings} style={{ background:'none', border:'none', color:'var(--text-2)', cursor:'pointer', fontSize:16, padding:'4px 8px' }}
...
function Ticker({ state }) {
  ...
  const items = [
    ...
    `SHARES ${fmtNum(state.shares?.acceptedCount || 0)}`,
      ...
      {sorted.length===0?(
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.4rem'}}>
          {sorted.map(w=>{
            ...
            const shareCount = w.sharesCount || 0;
            const rejCount = w.rejectedCount || 0;
            const totalCount = shareCount + rejCount || 1;
            ...
            return(
              <div key={w.name} style={{display:'flex',alignItems:'center',gap:'0.6rem',padding:'0.6rem 0.875rem',background:'var(--bg-raised)',border:`1px solid ${on?'rgba(57,255,106,0.12)':'transparent'}`,opacity:on?1:0.45}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontFamily:'var(--fm)',fontSize:'0.72rem',color:'var(--text-1)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                    {w.name}
                    {w.minerType && <span style={{fontFamily:'var(--fd)',fontSize:'0.54rem',letterSpacing:'0.1em',color:'var(--text-3)',marginLeft:6,textTransform:'uppercase'}}>{w.minerType}</span>}
                  </div>
                  <div style={{display:'flex',gap:8,alignItems:'center',marginTop:3}}>
                    <div style={{flex:1,height:2,background:'var(--bg-deep)',borderRadius:1,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${(shareCount/totalCount)*100}%`,background:'var(--green)',borderRadius:1}}/>
                    </div>
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:1}}>
                  <span style={{fontFamily:'var(--fm)',fontSize:'0.62rem',color:'var(--text-2)'}}>
                    <span style={{color:'var(--green)'}}>{fmtNum(shareCount)}</span>/<span style={{color:'var(--red)'}}>{rejCount}</span>
                  </span>
...
// ── Bitcoin Network (expanded with reward, price, fees) ──────────────────────
    ...
    <div style={card} className="fade-in">
      ...
      <div style={{display:'flex',flexDirection:'column',gap:'0.6rem'}}>
        <div style={{background:'var(--bg-raised)',border:'1px solid var(--border)',padding:'0.875rem'}}>
          <div style={{fontFamily:'var(--fd)',fontSize:'0.55rem',letterSpacing:'0.15em',color:'var(--text-2)',textTransform:'uppercase',marginBottom:6}}>Shares</div>
          <div style={{fontFamily:'var(--fd)',fontSize:'1.8rem',fontWeight:700,color:'var(--green)',lineHeight:1}}>{fmtNum(accepted)}</div>
          <div style={{fontFamily:'var(--fm)',fontSize:'0.7rem',color:'var(--text-2)',marginTop:6}}>
            <span style={{color:'var(--red)'}}>{rejected}</span> rejected
            {stale>0 && <> · <span style={{color:'var(--amber)'}}>{stale}</span> stale ({staleRate}%)</>}
          </div>
        </div>
        <div style={{background:'var(--bg-raised)',border:'1px solid var(--border)',padding:'0.875rem'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.35rem'}}>
          {sorted.map((w, i) => (
            <div key={w.name} style={{display:'flex',alignItems:'center',gap:'0.6rem',padding:'0.55rem 0.8rem',background:'var(--bg-raised)',border:`1px solid ${i===0?'rgba(245,166,35,0.3)':'var(--border)'}`}}>
              ...
              <span style={{fontFamily:'var(--fd)',fontSize:'0.8rem',fontWeight:600,color:i===0?'var(--amber)':'var(--cyan)'}}>
  ...
  return (
    <div style={card} className="fade-in">
      ...
      <p style={{fontSize:'0.55rem',color:'var(--text-3)',fontFamily:'var(--fm)',textAlign:'center',lineHeight:1.5,marginTop:'0.5rem'}}>
        most recent block finders — hashwar snapshot
      </p>
    </div>
  );
}
...
function SetupScreen({ onComplete }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'var(--bg-void)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'1rem'}}>
      <div style={{width:'100%',maxWidth:500,background:'var(--bg-surface)',border:'1px solid var(--border-hot)',padding:'2rem',boxShadow:'var(--glow-a)',position:'relative'}}>
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(6,7,8,0.88)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:'1rem'}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{width:'100%',maxWidth:460,background:'var(--bg-surface)',border:'1px solid var(--border-hot)',padding:'1.75rem',boxShadow:'var(--glow-a)',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <button onClick={submit} disabled={loading} style={{width:'100%',padding:'0.75rem',background:saved?'var(--green)':'var(--amber)',color:'#000',border:'none',fontFamily:'var(--fd)',fontSize:'0.8rem',fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',cursor:'pointer',opacity:loading?0.6:1,transition:'background 0.2s'}}>
...
// ── Default card order & registry ─────────────────────────────────────────────
const DEFAULT_ORDER = [
  'hashrate',   // Pool Hashrate (span 2)
  'workers',    // Connected Workers (span 2)
  'network',    // Bitcoin Network
  'odds',       // Block Probability
  'luck',       // Luck
  'retarget',   // Difficulty Retarget
  'shares',     // Share Stats
  'best',       // Best Shares Leaderboard
  'blocks',     // Blocks Found
  'topfinders', // Top Pool Finders
  'recent',     // Recent Network Blocks (span 2)
];
...
function loadOrder() {
  try {
    ...
    // Ensure any new cards added in updates still appear
    const merged = [...parsed];
...
export default function App() {
  ...
  // Card registry: id -> { spanTwo, element }
  const cards = {
    ...
    <>
      <div style={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
        ...
        <footer style={{borderTop:'1px solid var(--border)',padding:'0.6rem 1rem',display:'flex',justifyContent:'space-between',fontFamily:'var(--fd)',fontSize:'0.55rem',color:'var(--text-3)',letterSpacing:'0.08em',textTransform:'uppercase',gap:'0.5rem',flexWrap:'wrap'}}>
          <span>SoloStrike v1.2.2 — ckpool-solo</span>
