      ...
      {filtered.length === 0 ? (
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.4rem',maxHeight:400,overflowY:'auto'}}>
          {filtered.map(w=>{
            ...
            return(
              <div key={w.name} onClick={()=>onWorkerClick&&onWorkerClick(w)} style={{display:'flex',alignItems:'center',gap:'0.55rem',padding:'0.55rem 0.7rem',background:'var(--bg-raised)',border:`1px solid ${on?'rgba(57,255,106,0.12)':'transparent'}`,opacity:on?1:0.45,cursor:'pointer',transition:'background 0.15s', minWidth:0, overflow:'hidden'}}
                ...
                {/* Middle column: name on its own line, then miner type, then bar/last/diff row */}
                <div style={{flex:1,minWidth:0}}>
                  {/* Row 1: just the name (truncates with ellipsis if needed) */}
                  ...
                  {/* Row 2: miner type tag (below name, on its own line — no longer cramming with name) */}
                  {w.minerType && (
                  ...
                  {/* Row 3: progress bar + last share + diff */}
                ...
                {/* Right column — accepted/rejected counts, best diff, hashrate.
                    Tighter widths so the middle column has more breathing room. */}
    ...
    >
      ...
      <div style={{display:'flex', flexDirection:'column', gap:'0.875rem'}}>
        ...
        {/* Per-block odds — animated branching gold vein (v1.7.22 redesign).
            A stylized vein with branching tributaries fills with amber→gold
            from left to right based on `scale`. A bright pulse of light
            travels along the filled section every ~3s, evoking molten gold
            flowing through rock. Same data, way more visual character. */}
        <div>
          ...
          <svg viewBox="0 0 600 40" preserveAspectRatio="none"
            ...
            <defs>
              ...
              {/* Soft glow for the leading edge */}
              <filter id="veinGlow" x="-10%" y="-50%" width="120%" height="200%">
                <feGaussianBlur stdDeviation="1.4"/>
              </filter>
              {/* Clip path that grows L→R based on scale value, revealing the gold beneath */}
              <clipPath id="veinFillClip">
                <rect x="0" y="0" width={600 * scale} height="40">
                  <animate attributeName="width"
                    from={`${600 * scale * 0.97}`}
                    to={`${600 * scale * 1.03}`}
                    dur="2.4s" repeatCount="indefinite"
                    values={`${600*scale*0.97};${600*scale*1.03};${600*scale*0.97}`}/>
                </rect>
              </clipPath>
              {/* Animated mask for the traveling pulse of brightness along the vein */}
              <linearGradient id="veinPulse" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="rgba(255,255,255,0)"/>
                <stop offset="50%" stopColor="rgba(255,240,200,0.85)"/>
                <stop offset="100%" stopColor="rgba(255,255,255,0)"/>
                <animate attributeName="x1" values="-0.3;1.0" dur="3.6s" repeatCount="indefinite"/>
                <animate attributeName="x2" values="0;1.3"   dur="3.6s" repeatCount="indefinite"/>
              </linearGradient>
            </defs>

            {/* ── Background (dim, unmined) layer ──
                Main vein + 5 scattered tributaries branching at different
                angles and points along the main flow. */}
            <g stroke="rgba(245,166,35,0.18)" fill="none" strokeLinecap="round">
              {/* Main vein — diagonal-ish curve from lower-left to upper-right */}
              <path d="M 4 28 Q 90 32, 160 22 T 320 16 Q 420 12, 510 22 T 596 14"
                    strokeWidth="3.5"/>
              {/* Tributary 1 — branches steeply up from main near 1/4 */}
              <path d="M 100 30 Q 130 18, 175 4"   strokeWidth="1.4"/>
              {/* Tributary 2 — branches up-right near 1/3 */}
              <path d="M 180 21 Q 220 12, 270 6"   strokeWidth="1.8"/>
              {/* Tributary 3 — short downward fork near middle */}
              <path d="M 260 18 Q 285 28, 305 36"  strokeWidth="1.3"/>
              {/* Tributary 4 — branches down-right at 2/3 */}
              <path d="M 380 14 Q 430 22, 470 32"  strokeWidth="1.8"/>
              {/* Tributary 5 — short upward kick near end */}
              <path d="M 470 24 Q 510 14, 540 4"   strokeWidth="1.4"/>
              {/* Tributary 6 — tiny capillary near far right */}
              <path d="M 555 18 Q 575 26, 590 36"  strokeWidth="1.1"/>
            </g>

            {/* ── Foreground (lit, mined) layer — clipped to the scale-driven width ── */}
            <g clipPath="url(#veinFillClip)">
              ...
              {/* Bright pulse traveling along the lit main vein */}
              <g stroke="url(#veinPulse)" fill="none" strokeLinecap="round" filter="url(#veinGlow)">
                <path d="M 4 28 Q 90 32, 160 22 T 320 16 Q 420 12, 510 22 T 596 14"
                      strokeWidth="4.5"/>
              </g>
            </g>
  ...
  return (
    <>
     ...
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={`ss-app-main ${useCarousel ? 'ss-carousel-wrap' : ''}`}>
        <div
          ref={carouselRef}
