...
// ── Worker grid (reads diff-weighted shares) ──────────────────────────────────
      ...
      {sorted.length===0?(
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.4rem'}}>
          {sorted.map(w=>{
            ...
            // ckpool tracks DIFF-WEIGHTED work, not raw counts. Use those fields.
...
// ── Share stats (reads diff-weighted work, formats compact) ───────────────────
function ShareStats({ shares, hashrate, bestshare }) {
  const s = shares || {};
  // ckpool tracks difficulty-weighted work, not raw share counts.
  // Display the real work values with compact formatting (9.76 G, 6.44 M, etc.)
