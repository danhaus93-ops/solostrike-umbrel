...
export default function App() {
  ...
  // ── Stratum first-then-rotate (v1.7.17) ─────────────────────────────────
  // For first-time users (no host AND no workername configured yet), bring
  // the Stratum card to the front so the very first thing they see is the
  // setup form. Once they've configured both, mark "rotated" in localStorage
  // so we never auto-shuffle again — they're free to drag-to-reorder later
  // without us fighting them.
  const stratumIsConfigured = !!(loadStratumHost() && loadStratumWorkername());
  useEffect(() => {
    if (stratumIsConfigured && !loadStratumRotated()) {
      // Move Stratum to the END of the visible order (first time only)
      const newOrder = order.filter(id => id !== 'stratum').concat(['stratum']);
      saveOrder(newOrder);
      setOrder(newOrder);
      saveStratumRotated();
    }
  ...
  // Compute the rendered order. If user hasn't configured Stratum yet AND
  // hasn't been rotated yet, bring it to the front. Otherwise honor the
  // saved order from `baseOrder`.
  const renderableOrder = useMemo(() => {
    if (!stratumIsConfigured && !loadStratumRotated() && baseOrder.includes('stratum')) {
      return ['stratum', ...baseOrder.filter(id => id !== 'stratum')];
    }
    return baseOrder;
  }, [baseOrder, stratumIsConfigured]);

  // ── Carousel state (v1.7.17) ─────────────────────────────────────────────
  const isMobile = useIsMobile();
  const carouselEnabled = loadCarouselEnabled();
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        setActiveIndex(prev => prev === idx ? prev : Math.max(0, Math.min(renderableOrder.length - 1, idx)));
      });
    };
    ...
    <>
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={useCarousel ? 'ss-carousel-wrap' : ''}>
        ...
        <span>SoloStrike v1.7.17 — ckpool-solo{poolState?.privateMode && ' · 🔒 PRIVATE'}{minimalMode && ' · MIN'}</span>
