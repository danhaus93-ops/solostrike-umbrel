...
const MINIMAL_PRESET  = ['hashpulse', 'workers', 'blocks'];
const DEFAULT_PRESET  = ['hashpulse', 'workers', 'stratum', 'vein', 'network', 'shares', 'best', 'closestcalls', 'blocks'];
...
function migrateCardIds(arr) {
  ...
  for (const id of arr) {
    ...
    //   'hashrate' -> 'hashpulse'   (v1.7.20: merged with pulse into combined card)
    //   'pulse'    -> 'hashpulse'   (v1.7.20: merged with hashrate into combined card)
    let next = id;
    ...
    compact ? (
      <>
        ...
        <div
          ...
          style={{ cursor: onOpenStrikers ? 'pointer' : 'default', position:'relative' }}
        ...
        {/* Footer tagline — single line in compact */}
        <div style={{
          borderTop:'1px dashed rgba(245,166,35,0.18)',
          paddingTop:'0.4rem',
...
function HashPulsePanel({ history, week, current, networkStats, onOpenSettings, onOpenStrikers }) {
  return (
    <div style={{...card, position:'relative', minWidth:0, maxWidth:'100%', overflow:'hidden'}} className="fade-in">
      ...
      {/* The 100% SOLO stamp from PulsePanel — keep at card level for the iconic corner badge */}
      <StampSolo/>
    </div>
  );
}
...
// ── Layout helpers ────────────────────────────────────────────────────────────
const DEFAULT_ORDER = ['hashpulse','workers','stratum','vein','network','node','luck','retarget','shares','best','closestcalls','blocks','topfinders','recent'];
function loadOrder() {
  try {
    ...
    <>
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={useCarousel ? 'ss-carousel-wrap' : ''}>
        ...
        <span>SoloStrike v1.7.20 — ckpool-solo{poolState?.privateMode && ' · 🔒 PRIVATE'}{minimalMode && ' · MIN'}</span>
