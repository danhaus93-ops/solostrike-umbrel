...
export default function App() {
  ...
  // For first-time users, brings Stratum card to position 0; once the user
  // has both host AND workername configured, rotates Stratum to the END of
  // the order (one-time, marked in localStorage so it never auto-shuffles
  // again — preserving any future drag-to-reorder choice).
  useEffect(() => {
    const configured = !!(loadStratumHost() && loadStratumWorkername());
    if (configured && !loadStratumRotated()) {
  ...
  // hasn't been rotated yet, bring it to the front. The actual rotation
  // (move-to-end on setup completion) happens in a useEffect higher up.
    ...
    <>
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={useCarousel ? 'ss-carousel-wrap' : ''}>
        ...
        <span>SoloStrike v1.7.18 — ckpool-solo{poolState?.privateMode && ' · 🔒 PRIVATE'}{minimalMode && ' · MIN'}</span>
