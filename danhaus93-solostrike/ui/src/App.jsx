...
const LS_PERFORMANCE_MODE = 'ss_performance_mode_v1'; // v1.11.38: Performance Mode toggle
...
// v1.11.38: Performance Mode — replaces animated Pulse/Hunt canvases with
...
const _ssDebug = {
  ...
  // v1.11.38 — ticker / stutter diagnosis streams
  ...
  // v1.11.38 — WebSocket spawn tracking (duplicate-socket bug detection)
...
// v1.11.38 FIX: expose _ssDebug to window so cross-module instrumentation
...
const Ticker = React.memo(function Ticker({ snapshotText, enabled, speedSec }) {
  ...
  useEffect(() => {
    ...
    const step = (t) => {
      ...
      // v1.11.38: instrument frames + stalls for diagnostic dump
...
function NonceField({ hashrate, huntAnim, performanceMode }) {
  ...
  // v1.11.38: Performance Mode ref — checked inside the rAF body to bail
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // v1.11.38: Performance Mode short-circuit. Skip all draw work — the
  ...
  return (
    <div ref={containerRef} style={{
      ...
      {/* v1.11.38: Performance Mode static frame — overlays all canvases
...
// v1.11.38: memoized to skip re-renders when props unchanged across WS broadcasts
...
function DisplayTab({ stripSettings, onStripSettingsChange, tickerSettings, onTickerSettingsChange, minimalMode, onMinimalModeChange, performanceMode, onPerformanceModeChange, visibleCards, onVisibleCardsChange, carouselEnabled, onCarouselChange }) {
  ...
  return (
    <>
      ...
      {/* v1.11.38: Performance Mode — freezes decorative animations while
...
// ── v1.11.38 ── Static Pulse Strikes (Performance Mode overlay) ─────────────
...
// ── v1.11.38 ── Static Pulse Strikes — DOM divs ────────────────────────────
...
// ── v1.11.38 — Static Pulse Mesh helpers (restored) ────────────────────────
...
const PulsePanel = React.memo(function PulsePanel_Impl({ networkStats, onOpenSettings, onOpenStrikers, pulseAnim = 'block', performanceMode = false, compact = false, poolPin = null, onPoolPinChange = null, lastShareAt = null, acceptedCount = 0, workers = null }) {
  ...
  // v1.11.38 — Pinch-zoom + pan state for Performance Mode static
  ...
  // v1.11.38: Performance Mode ref — same pattern as NonceField. Live
  ...
  const handleCanvasTap = useCallback((e) => {
    ...
    // v1.11.38: Performance Mode uses a flat equirectangular map (not a
    ...
    if (perfModeRef.current) {
      // v1.11.38: Suppress tap-to-pin if the user just finished a pinch
      // gesture (didPinchRef gets set on 2-finger touchend).
      if (didPinchRef.current) {
        didPinchRef.current = false;
        return;
      }
      // v1.11.38: Inverse-transform click coords if user has zoomed/panned.
      ...
      // v1.11.38: Standard equirectangular [-90, +90] — keeps pin
  ...
  // v1.11.38 — Pinch-zoom + pan touch handlers for the Performance Mode
  ...
  useEffect(() => {
    ...
    // v1.11.38: Click → place pin in Performance Mode (when pulseAnim is
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // v1.11.38: Performance Mode short-circuit. The static <img> overlay
  ...
  return (
    <>
      ...
      <div style={{position:'relative'}}>
        ...
        <div ref={containerRef} style={{
          width:'100%',
          // v1.11.38: Compact layout — mirror the live globe sizing.
          ...
          />
          {/* v1.11.38: Performance Mode static Pulse map (compact layout). */}
          {/* v1.11.38 — Pinch-zoom wrapper (same as full layout below). */}
          {performanceMode && (
            <div
          ...
          {/* v1.11.38: DOM strike markers — CSS-keyframe animated rings
    ...
    <div style={{...card, position:'relative', minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      ...
      <div
        style={{
          // v1.11.38: Restored flex:1 always — the inner containerRef now
      ...
      <div ref={containerRef} style={{
        width:'100%',
        // v1.11.38: Mirror the LIVE GLOBE sizing exactly — flex:1 with
        ...
        />
        {/* v1.11.38: Performance Mode static Pulse map. Overlays globe and
            ...
            pin placement. */}
        {/* v1.11.38 — Pinch-zoom + pan wrapper. Map, mesh, and SVG strikes
    ...
    try {
      const bundle = {
        ...
        streams: {
          ...
          // v1.11.38 — stutter diagnosis streams
...
export default function App() {
  ...
  const [performanceMode, setPerformanceMode] = useState(loadPerformanceMode()); // v1.11.38
  ...
  const onPerformanceModeChange = (v) => { setPerformanceMode(v); savePerformanceMode(v); }; // v1.11.38
    ...
    <>
     <div ref={headerRef} className="ss-app-header" style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={useCarousel ? 'ss-carousel-wrap' : ''}>
        ...
        <span>SoloStrike v1.11.38 — ckpool-solo{poolState?.privateMode && ' · 🔒 PRIVATE'}{minimalMode && ' · MIN'}</span>
