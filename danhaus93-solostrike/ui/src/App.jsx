...
const DEFAULT_PRESET  = ['hashrate', 'workers', 'stratum', 'vein', 'network', 'shares', 'best', 'closestcalls', 'blocks'];
...
function CopyablePort({ health, port, ssl }) {
  ...
  const onCopy = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    const host = (typeof window !== 'undefined' && window.location && window.location.hostname) || 'umbrel.local';
...
function HashrateChart({ history, week, current }) {
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden'}} className="fade-in">
      ...
      <div style={{ fontFamily:'var(--fd)', fontSize:'2.6rem', fontWeight:700, color:'var(--amber)', letterSpacing:'0.01em', lineHeight:1, textShadow:'0 0 30px rgba(245,166,35,0.35)', marginBottom:'0.8rem', display:'flex', alignItems:'baseline', flexWrap:'wrap', gap:'0.4rem' }}>
        <span>{p0}<span style={{ fontSize:'1rem', color:'var(--amber-dim)', marginLeft:4 }}>{p1}</span></span>
  ...
  return (
    ...
    >
      ...
      <div style={{display:'flex', flexDirection:'column', gap:'0.875rem'}}>
        ...
        {/* Orbital gauge — same as old Strike Odds, centerpiece */}
        <div style={{display:'flex', justifyContent:'center'}}>
          <div style={{position:'relative', width:110, height:110, display:'flex', alignItems:'center', justifyContent:'center'}}>
            ...
            <div style={{textAlign:'center'}}>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.78rem', fontWeight:700, color:'var(--amber)', lineHeight:1.2}}>
                {perBlock>0 ? `${(perBlock*100).toExponential(1)}%` : '—'}
              </div>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.58rem', color:'var(--text-2)', letterSpacing:'0.1em', textTransform:'uppercase', marginTop:3}}>
                per block
              </div>
            </div>
          </div>
        ...
        {/* Fee tier strip — Fast / Mid / Low (sat/vB) */}
        ...
        {/* Stats grid — 2x2 layout for density */}
...
// ── Stratum Connection card (v1.7.6) ─────────────────────────────────────
// One-stop "how do I connect a miner" card. Shows three stratum endpoints
// with copy buttons. HOST resolves from window.location.hostname so it works
// over LAN, Tailscale, or local. Worker name placeholder, "x" password.
function StratumPanel({ payoutAddress, stratumHealth }) {
  ...
  const addrShort = payoutAddress
    ...
    : 'YOUR_BTC_ADDRESS';
  const userExample = payoutAddress
  ...
  const Row = ({ port, label, accent, sslHint }) => {
      ...
      <div style={{
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6}}>
          <div style={{display:'flex', alignItems:'center', gap:7}}>
            ...
            {sslHint && (
              <span style={{fontFamily:'var(--fd)', fontSize:'0.55rem', color:'var(--cyan)', letterSpacing:'0.12em', background:'rgba(0,255,209,0.08)', border:'1px solid var(--cyan)', padding:'1px 6px'}}>
                🔒 SSL
              </span>
            )}
          </div>
          <span style={{fontFamily:'var(--fd)', fontSize:'0.65rem', color:ps.color, letterSpacing:'0.08em'}}>
            {ps.dot} :{port}
          </span>
        </div>
        <div style={{display:'flex', alignItems:'center', gap:6}}>
          ...
          <button
            onClick={() => copy(url, `port${port}`)}
            style={{
              background: copied === `port${port}` ? 'rgba(57,255,106,0.1)' : 'none',
              border: `1px solid ${copied === `port${port}` ? 'var(--green)' : 'var(--border)'}`,
              color: copied === `port${port}` ? 'var(--green)' : 'var(--amber)',
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden'}} className="fade-in">
      ...
      <Row port="3333" label="ASIC PORT"   accent="var(--amber)" />
      <Row port="3334" label="HOBBY PORT"  accent="var(--text-1)" />
      <Row port="4333" label="SECURE PORT" accent="var(--cyan)" sslHint />
      ...
      <div style={{
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6}}>
          <span style={{fontFamily:'var(--fd)', fontSize:'0.6rem', letterSpacing:'0.12em', color:'var(--text-2)', textTransform:'uppercase'}}>USER</span>
          <button
            onClick={() => payoutAddress && copy(userExample, 'user')}
            disabled={!payoutAddress}
            style={{
              background: copied === 'user' ? 'rgba(57,255,106,0.1)' : 'none',
              border: `1px solid ${copied === 'user' ? 'var(--green)' : 'var(--border)'}`,
              color: copied === 'user' ? 'var(--green)' : (payoutAddress ? 'var(--amber)' : 'var(--text-2)'),
              fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em',
              padding:'3px 8px', cursor: payoutAddress ? 'pointer' : 'not-allowed',
              textTransform:'uppercase',
              opacity: payoutAddress ? 1 : 0.5,
            }}>
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6, marginTop:9}}>
          <span style={{fontFamily:'var(--fd)', fontSize:'0.6rem', letterSpacing:'0.12em', color:'var(--text-2)', textTransform:'uppercase'}}>PASS</span>
          <button
            onClick={() => copy('x', 'pass')}
            style={{
              background: copied === 'pass' ? 'rgba(57,255,106,0.1)' : 'none',
              border: `1px solid ${copied === 'pass' ? 'var(--green)' : 'var(--border)'}`,
              color: copied === 'pass' ? 'var(--green)' : 'var(--amber)',
              fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em',
        ...
        <div style={{
          ...
          x <span style={{color:'var(--text-2)'}}>(or d=XXXX to set fixed difficulty)</span>
        </div>
      </div>
      ...
      <div style={{
        ...
        Connect any Stratum V1 miner. Worker name is whatever you set on the device — it'll show up in <span style={{color:'var(--amber)', fontWeight:600}}>The Crew</span> automatically.
        {!payoutAddress && (
          <span style={{display:'block', marginTop:5, color:'var(--amber)', fontWeight:600}}>
...
// ── Layout helpers ────────────────────────────────────────────────────────────
const DEFAULT_ORDER = ['hashrate','workers','stratum','vein','network','node','luck','retarget','shares','best','closestcalls','blocks','topfinders','recent','pulse'];
function loadOrder() {
  try {
...
export default function App() {
  ...
  // ── Update banner state ──────────────────────────────────────────────
  // The service worker dispatches 'sw:update-available' when a new bundle
  // has been installed in the background. We compare server-reported
  // composeVersion against what's bundled in this build to decide whether
  // a "soft" service-worker reload is enough or whether the user has to
  // open Umbrel to apply infrastructure changes.
  ...
  // Decide which update tier (if any) to show.
  // - 'hard' if server's composeVersion > our built composeVersion
  // - 'soft' if SW says a new bundle is ready
  // - null  if neither
  const updateTier = useMemo(() => {
    ...
    if (swUpdateReady) return 'soft';
    return null;
  }, [poolState?.composeVersion, swUpdateReady]);
  ...
  const applySoftUpdate = useCallback(() => {
    ...
    // controllerchange handler in index.html will reload the page
    ...
    <>
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding:'0.65rem'}}>
        ...
        <span>SoloStrike v1.7.13 — ckpool-solo{poolState?.privateMode && ' · 🔒 PRIVATE'}{minimalMode && ' · MIN'}</span>
