...
function SettingsModal({ onClose, saveConfig, currentConfig }) {
  ...
  const submit = async () => {
    ...
    try{
      ...
      if(trimmed) {
        if(!isValidBtcAddress(trimmed)) { setError("That doesn't look like a valid Bitcoin address."); setLoading(false); return; }
        p.payoutAddress = trimmed;
      }
      await saveConfig(p);
      setSaved(true);setAddr('');setTimeout(()=>setSaved(false),3000);
    }
    ...
    <>
      <div style={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
        ...
        <footer style={{borderTop:'1px solid var(--border)',padding:'0.6rem 1.5rem',display:'flex',justifyContent:'space-between',fontFamily:'var(--fd)',fontSize:'0.58rem',color:'var(--text-3)',letterSpacing:'0.08em',textTransform:'uppercase'}}>
          <span>SoloStrike v1.2.1 — ckpool-solo</span>
