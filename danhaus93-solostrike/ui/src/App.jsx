      ...
      {filtered.length === 0 ? (
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0.4rem',flex:1,minHeight:0,maxHeight:340,overflowY:'auto', WebkitMaskImage:'linear-gradient(to bottom, transparent 0, black 12px, black calc(100% - 12px), transparent 100%)', maskImage:'linear-gradient(to bottom, transparent 0, black 12px, black calc(100% - 12px), transparent 100%)'}}>
          {filtered.map(w=>{
            ...
            return(
              <div key={w.name} onClick={()=>onWorkerClick&&onWorkerClick(w)} style={{display:'flex',alignItems:'center',gap:'0.55rem',padding:'0.55rem 0.7rem',background:'var(--bg-raised)',border:`1px solid ${on?'rgba(57,255,106,0.12)':'transparent'}`,opacity:on?1:0.45,cursor:'pointer',transition:'background 0.15s', minWidth:0}}
                ...
                <span title={w.minerType||'Unknown'} style={{fontSize:13,color:on?'var(--cyan)':'var(--text-3)',width:14,textAlign:'center',flexShrink:0}}>{icon}</span>
                {/* Middle column: name on its own line, miner type below it,
                    then a tight progress bar + last-share row. Each row truncates
                    independently so nothing fights for horizontal space. */}
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontFamily:'var(--fm)',fontSize:'0.78rem',color:'var(--text-1)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontWeight:500}} title={w.name}>
                    {disp}
                  </div>
                  {w.minerType && (
                    <div style={{fontFamily:'var(--fd)',fontSize:'0.5rem',letterSpacing:'0.1em',color:'var(--text-3)',textTransform:'uppercase',marginTop:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                      {w.minerType}
                    </div>
                  )}
                  <div style={{display:'flex',gap:6,alignItems:'center',marginTop:4,minWidth:0}}>
                    <div style={{flex:1,height:2,background:'var(--bg-deep)',borderRadius:1,overflow:'hidden',minWidth:0}}>
                      <div style={{height:'100%',width:`${(workAccepted/totalWork)*100}%`,background:'var(--green)',borderRadius:1}}/>
                    </div>
                    <span style={{fontFamily:'var(--fm)',fontSize:'0.52rem',color:'var(--text-3)',whiteSpace:'nowrap',flexShrink:0}}>{lastShareAgo}</span>
                  </div>
                </div>
                {/* Right column — hashrate (big), accepted/rejected, best diff.
                    Tighter minWidth so the middle column has more breathing room. */}
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:1, flexShrink:0, minWidth:54}}>
                  <span style={{fontFamily:'var(--fd)',fontSize:'0.78rem',fontWeight:600,color:on?'var(--amber)':'var(--text-2)',whiteSpace:'nowrap'}}>
                    {on?fmtHr(w.hashrate):'offline'}
                  </span>
                  <span style={{fontFamily:'var(--fm)',fontSize:'0.55rem',color:'var(--text-2)',whiteSpace:'nowrap'}}>
                    <span style={{color:'var(--green)'}}>{fmtDiff(workAccepted)}</span>{workRejected>0 && <>/<span style={{color:'var(--red)'}}>{fmtDiff(workRejected)}</span></>}
                  </span>
                  {w.bestshare>0 && <span style={{fontFamily:'var(--fm)',fontSize:'0.5rem',color:'var(--amber)',whiteSpace:'nowrap'}}>best {fmtDiff(w.bestshare)}</span>}
                </div>
              </div>
            );
          })}
...
function ClosestCallsPanel({ closestCalls, aliases }) {
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden'}} className="fade-in">
      ...
      <div style={{display:'flex', flexDirection:'column', gap:'0.35rem', flex:1, minHeight:0, maxHeight:380, overflowY:'auto', WebkitMaskImage:'linear-gradient(to bottom, transparent 0, black 12px, black calc(100% - 12px), transparent 100%)', maskImage:'linear-gradient(to bottom, transparent 0, black 12px, black calc(100% - 12px), transparent 100%)'}}>
        {list.map((c, i) => {
      ...
      <div style={{display:'flex', flexDirection:'column', gap:'0.875rem'}}>
        ...
        <div>
          ...
          <svg viewBox="0 0 600 60" preserveAspectRatio="none"
            ...
            {(() => {
              const branches = [
                // ── MAIN TRUNK — heavy left, sweeping up-right, tapering to fine right ──
                { d: "M 2 38 Q 25 36, 50 32 Q 75 28, 105 26 Q 145 24, 185 22", w: 3.5 },  // heavy anchor
                { d: "M 185 22 Q 230 21, 280 20 Q 330 19, 380 19 Q 430 19, 480 18", w: 1.8 },
                { d: "M 480 18 Q 510 17, 540 16 Q 565 15, 588 14", w: 1.0 },  // tapering tail

                // ── LEFT CLUMP - chunky junction (matches the heavy left silhouette) ──
                { d: "M 8 38 Q 12 22, 16 6",          w: 2.2 },  // tall vertical anchor
                { d: "M 18 36 Q 24 22, 32 6",         w: 1.9 },
                { d: "M 32 34 Q 42 20, 56 4",         w: 1.7 },
                { d: "M 50 32 Q 60 20, 72 8",         w: 1.5 },
                { d: "M 70 30 Q 82 20, 95 12",        w: 1.4 },
                { d: "M 14 38 Q 10 48, 6 58",         w: 1.3 },  // down-left
                { d: "M 36 36 Q 38 48, 42 58",        w: 1.2 },  // down

                // ── MAJOR MID-LEFT FORK (the big up-cluster around 30-40% in ref) ──
                { d: "M 130 25 Q 145 14, 162 4",      w: 1.6 },
                { d: "M 150 24 Q 165 12, 180 2",      w: 1.4 },
                { d: "M 170 23 Q 185 12, 200 4",      w: 1.3 },
                { d: "M 195 22 Q 210 12, 226 4",      w: 1.2 },

                // ── LEFT CLUMP recursive sub-forks (lots of small ones) ──
                { d: "M 18 18 Q 22 14, 28 12",        w: 0.9 },
                { d: "M 22 12 Q 26 8, 33 8",          w: 0.7 },
                { d: "M 26 4  L 32 1",                w: 0.5 },
                { d: "M 36 16 Q 42 10, 50 12",        w: 0.8 },
                { d: "M 44 6  Q 48 3, 54 4",          w: 0.6 },
                { d: "M 58 14 Q 64 8, 72 10",         w: 0.8 },
                { d: "M 60 4  L 66 1",                w: 0.5 },
                { d: "M 76 8  Q 82 4, 90 6",          w: 0.7 },
                { d: "M 88 14 Q 94 10, 102 12",       w: 0.7 },
                { d: "M 92 4  L 98 1",                w: 0.5 },

                // ── Mid-fork sub-branches ──
                { d: "M 140 16 Q 148 10, 156 12",     w: 0.7 },
                { d: "M 158 8  Q 164 4, 172 6",       w: 0.6 },
                { d: "M 172 14 Q 180 8, 188 10",      w: 0.7 },
                { d: "M 184 4  L 190 0",              w: 0.5 },
                { d: "M 198 8  Q 206 4, 214 6",       w: 0.6 },
                { d: "M 215 12 L 222 6",              w: 0.5 },

                // ── MIDDLE section forks (chaotic, off main trunk) ──
                { d: "M 230 21 Q 250 12, 275 6",      w: 1.1 },
                { d: "M 260 20 Q 282 14, 305 8",      w: 1.0 },
                { d: "M 290 20 Q 305 28, 318 38",     w: 1.0 },  // down
                { d: "M 320 19 Q 340 12, 365 6",      w: 1.0 },
                { d: "M 350 19 Q 365 26, 378 36",     w: 0.9 },  // down
                { d: "M 245 18 L 256 12",             w: 0.6 },
                { d: "M 280 14 L 292 8",              w: 0.5 },
                { d: "M 312 14 L 322 8",              w: 0.5 },
                { d: "M 358 12 L 368 6",              w: 0.5 },

                // ── RIGHT section (sparser, finer) ──
                { d: "M 395 19 Q 415 12, 438 6",      w: 0.9 },
                { d: "M 425 19 Q 442 26, 458 36",     w: 0.8 },  // down
                { d: "M 450 18 Q 470 12, 492 6",      w: 0.8 },
                { d: "M 415 13 L 425 7",              w: 0.5 },
                { d: "M 462 12 L 472 6",              w: 0.5 },
                { d: "M 480 14 L 490 8",              w: 0.4 },

                // ── FAR-RIGHT tail (very fine capillaries) ──
                { d: "M 510 17 Q 525 12, 540 8",      w: 0.6 },
                { d: "M 535 16 L 545 10",             w: 0.4 },
                { d: "M 555 16 Q 568 12, 580 8",      w: 0.5 },
                { d: "M 565 14 L 573 8",              w: 0.4 },
                { d: "M 545 18 L 552 24",             w: 0.4 },  // tiny down
                { d: "M 580 14 L 588 9",              w: 0.4 },
                { d: "M 595 14 L 600 11",             w: 0.3 },
              ];
              return (
                <>
        ...
        {/* Block reward hero — subsidy + fees breakdown (UNCHANGED) */}
        <div style={{
        ...
        {(feeFast || feeMid || feeLow) && (
          <div style={{display:'flex', gap:6}}>
            <div style={{flex:1, background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.5rem 0.4rem', textAlign:'center'}}>
        ...
        {/* Stats grid — 2x2 layout for density (UNCHANGED) */}
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:6}}>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.55rem 0.6rem'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--text-2)', textTransform:'uppercase'}}>EXPECTED</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.85rem', color:'var(--amber)', fontWeight:700, marginTop:3}}>
              {fmtOdds(expectedDays)}
            </div>
          </div>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.55rem 0.6rem'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--text-2)', textTransform:'uppercase'}}>POOL SHARE</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.85rem', color:'var(--text-1)', fontWeight:700, marginTop:3}}>
              {netHashrate>0 && hashrate>0 ? `${((hashrate/netHashrate)*100).toExponential(2)}%` : '—'}
            </div>
          </div>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.55rem 0.6rem'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--text-2)', textTransform:'uppercase'}}>DAILY CHANCE</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.85rem', color:'var(--text-1)', fontWeight:700, marginTop:3}}>
              {perDay>0 ? fmtPct(perDay*100, 4) : '—'}
            </div>
          </div>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.55rem 0.6rem'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--text-2)', textTransform:'uppercase'}}>EXP. DAILY</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.85rem', color:'var(--cyan)', fontWeight:700, marginTop:3}}>
              {expectedDailySats > 0 ? `${expectedDailySats.toLocaleString()} sats` : '—'}
            </div>
          </div>
        </div>
...
function BlockFeed({ blocks, blockAlert, compact = false }) {
  const inner = (
    <>
      ...
      {!(blocks||[]).length?(
        <div style={{textAlign:'center',padding: compact ? '0.9rem' : '1.5rem',border:'1px dashed var(--border)',color:'var(--text-2)',fontSize:'0.75rem',fontFamily:'var(--fd)'}}>No gold struck yet.<br/><span style={{color:'var(--amber)',fontSize:'0.68rem'}}>Keep digging ⛏</span></div>
      ):(
...
export default function App() {
  ...
  // v1.7.22-iter: tag the body with the active layout mode so CSS can apply
  // different sizing rules. Carousel mode locks body height to its container
  // (fixes Umbrel iframe leaving empty space at the bottom). Vertical mode
  // lets the body grow with content for natural page scrolling.
  useEffect(() => {
  ...
  return (
    <>
     <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
      ...
      <main style={{padding: useCarousel ? 0 : '0.65rem'}} className={useCarousel ? 'ss-carousel-wrap' : ''}>
        ...
        <footer style={{borderTop:'1px solid var(--border)',padding:'0.6rem 0.75rem',paddingBottom:'calc(0.6rem + env(safe-area-inset-bottom))',display:'flex',justifyContent:'space-between',alignItems:'center',fontFamily:'var(--fd)',fontSize:'0.5rem',color:'var(--text-3)',letterSpacing:'0.06em',textTransform:'uppercase',gap:'0.5rem',flexWrap:'nowrap',width:'100%',maxWidth:'100%',boxSizing:'border-box',whiteSpace:'nowrap',position:'sticky',bottom:0,background:'rgba(6,7,8,0.92)',backdropFilter:'blur(10px)',WebkitBackdropFilter:'blur(10px)',zIndex:50}}>
