...
export function usePool() {
    ...
    ws.onmessage = e => {
      try {
        ...
        if (msg.type === 'STATE_UPDATE') {
          // v1.11.39: compact WS broadcasts strip 4 heavy fields to keep
          ...
          setState(p => {
            ...
            // keep the old object.
            //
            // v1.11.39: BUT also apply peerHeartbeats updates to the preserved
            ...
            // v1.11.39: shares.spsHistory works just like hashrate.history —
            ...
            // v1.11.39: workers — compact broadcasts strip per-worker
            ...
            // v1.11.39: for hashrate: compact mode ships current+averages
            ...
            if (newData.hashrate) {
              ...
              // v1.11.39 SAFETY LOG: if we receive a tail with a huge gap
              ...
              // v1.11.39: gap-warned flag resets on any successful contiguous
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      ...
      clearTimeout(retryRef.current);
      retryCount.current = 0;
      // v1.11.39 FIX — duplicate-socket bug. ws.close() is async: it puts
      ...
      // v1.11.39: tag the spawn reason so wsSpawnLog explains each event
      if (typeof window !== 'undefined' && window._ssDebug) {
