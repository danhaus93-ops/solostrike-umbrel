...
const DEF = { config:{poolName:'SoloStrike',hasAddress:false}, status:'loading', hashrate:{current:0,history:[]}, workers:[], shares:{accepted:0,rejected:0,stale:0}, blocks:[], network:{height:0,difficulty:0,hashrate:0}, odds:{perBlock:0,expectedDays:null}, uptime:Date.now() };
...
export function usePool() {
  ...
  useEffect(() => { fetch('/api/state').then(r=>r.json()).then(d=>setState(d)).catch(()=>{}); }, []);
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = e => {
      try {
        ...
        else if (msg.type==='BLOCK_FOUND') { setBlockAlert(msg.data); setTimeout(()=>setBlockAlert(null),8000); }
        else if (msg.type==='CONFIG_UPDATED') setState(p=>({...p,config:{...p.config,...msg.data}}));
      } catch {}
    };
