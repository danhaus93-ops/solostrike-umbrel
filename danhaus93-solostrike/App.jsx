...
// v1.11.41: Performance Mode — replaces animated Pulse/Hunt canvases with
...
const _ssDebug = {
  ...
  // v1.11.41 — ticker / stutter diagnosis streams
  ...
  // v1.11.41 — WebSocket spawn tracking (duplicate-socket bug detection)
...
// v1.11.41 FIX: expose _ssDebug to window so cross-module instrumentation
...
function UpdateBanner({ tt = (x) => x, tier, urgency, version, notes, expanded, onToggleExpanded, onApply, onDismiss }) {
  ...
  if (isCritical) {
    ...
    glyph  = (
      ...
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink:0, color:'var(--cyan)', filter:'drop-shadow(0 0 4px rgba(0,255,209,0.4))' }}>
        <path d={tt("M14.7 6.3a4 4 0 0 0-5.4 5.4l-6 6a1.4 1.4 0 1 0 2 2l6-6a4 4 0 0 0 5.4-5.4l-2.4 2.4-2-2 2.4-2.4z")}/>
      </svg>
    );
  } else {
    ...
    glyph  = (
      ...
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink:0, color:'var(--amber)', filter:'drop-shadow(0 0 6px rgba(var(--amber-rgb),0.55))', animation:'pulse 2.2s ease-in-out infinite', willChange:'opacity' }}>
        <path d={tt("M13 2 L4 14 L11 14 L10 22 L20 9 L13 9 L13 2 Z")}/>
      </svg>
    );
  }
      ...
      <div style={{
        ...
        {!isCritical && (
          <button
            ...
            title={tt("Dismiss until next version")}
          >×</button>
        )}
      </div>
          ...
          {isHard && (
            <div style={{
              ...
              <div style={{ color: fg, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.3rem' }}>
                {tt('Update via Umbrel app store:')}
              </div>
              <div style={{ marginBottom: '0.2rem' }}>{tt('1. Open Umbrel on any device')}</div>
              <div style={{ marginBottom: '0.2rem' }}>{tt('2. Go to App Store → Community Store')}</div>
              <div style={{ marginBottom: '0.2rem' }}>{tt('3. Find SoloStrike → tap')} <span style={{ color:'var(--amber)' }}>{tt('Update')}</span></div>
              <div>{tt('4. Mining keeps hashing through the update')}</div>
            </div>
          )}
          {notes && (
            <div style={{
...
// ── Header ────────────────────────────────────────────────────────────────────
function Header({ tt = (x) => x, connected, status, onSettings, privateMode, minimalMode, performanceMode, zmq, blocksFound }) {
  const now = useNow(30000);
  const statusMap = { running:{c:'var(--green)',t:tt('MINING')}, mining:{c:'var(--green)',t:tt('MINING')}, no_address:{c:'var(--amber)',t:tt('SETUP')}, setup:{c:'var(--amber)',t:tt('SETUP')}, starting:{c:'var(--amber)',t:tt('STARTING')}, error:{c:'var(--red)',t:tt('ERROR')}, loading:{c:'var(--text-2)',t:'...'} };
  ...
  return (
    <header style={{ ...STRIP_FULL_WIDTH, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 0.5rem', minHeight:58, borderBottom:'1px solid var(--border)', gap:'0.4rem' }}>
      <div style={{ display:'flex', alignItems:'center', gap:'0.4rem', minWidth:0, flex:1, flexWrap:'wrap' }}>
        ...
        <span style={{ fontFamily:'var(--fd)', fontSize:'0.92rem', fontWeight:700, letterSpacing:'0.06em', color:'var(--amber)', textTransform:'uppercase', flexShrink:0 }}>{tt('SoloStrike')}</span>
        {!minimalMode && (
          <>
            ...
            <ZmqBadge tt={tt} zmq={zmq}/>
            {privateMode && (
              <span title={tt("Private Mode")} style={{ display:'inline-flex', alignItems:'center', gap:3, color:'var(--cyan)', fontFamily:'var(--fd)', fontSize:'0.54rem', letterSpacing:'0.12em', textTransform:'uppercase', textShadow:'0 0 6px rgba(0,255,209,0.4)', animation:'pulse 3s ease-in-out infinite', willChange:'opacity', flexShrink:0, marginLeft:4 }}>🔒</span>
            )}
            {/* Strikes counter — total blocks found by this install */}
            {blocksFound != null && (
              <span key={strikesPulseKey} className={strikesPulseKey > 0 ? 'ss-strikes-celebrate' : ''} title={tt("Total blocks struck")} style={{ display:'inline-flex', alignItems:'center', gap:3, fontFamily:'var(--fd)', fontSize:'0.6rem', letterSpacing:'0.1em', color: blocksFound > 0 ? 'var(--amber)' : 'var(--text-2)', textShadow: blocksFound > 0 ? '0 0 6px rgba(var(--amber-rgb),0.5)' : 'none', flexShrink:0, marginLeft:4 }}>
                {tt('STRIKES')} <span style={{fontWeight:700}}>{blocksFound}</span>{blocksFound > 0 && <span>⚡</span>}
              </span>
            )}
...
const Ticker = React.memo(function Ticker({ pillsSource, enabled, speedSec, lang = 'en' }) {
  ...
  useEffect(() => {
    ...
    const pxPerSec = () => containerWidth() / speed;
...
// ── Latest Block strip ────────────────────────────────────────────────────────
function LatestBlockStrip({ tt = (x) => x, netBlocks, blockReward }) {
  ...
  return (
    <div className="ss-hide-scrollbar" style={{
      ...
      <span style={{display:'inline-flex', alignItems:'center', gap:6, flexShrink:0}}>
        ...
        <span style={{color:'var(--amber)', fontWeight:700}}>{tt('LATEST BLOCK')}</span>
      </span>
      ...
      <span style={{color: latest.isSolo?'var(--amber)':'var(--text-1)', fontWeight:600, flexShrink:0}}>
        {latest.pool}{latest.isSolo && <span style={{marginLeft:6, fontSize:'0.52rem', border:'1px solid var(--amber)', padding:'1px 4px'}}>{tt('SOLO')}</span>}
      </span>
      <span style={{color:'var(--text-2)', flexShrink:0}}>·</span>
...
// ── Sync warning banner ───────────────────────────────────────────────────────
function SyncWarningBanner({ tt = (x) => x, sync }) {
  ...
  return (
    <div className="ss-hide-scrollbar" style={{
      ...
      <span style={{fontWeight:700, animation:'pulse 2s ease-in-out infinite', willChange:'opacity', flexShrink:0}}>{tt('⚠ BITCOIN CORE SYNCING')}</span>
      <span style={{color:'var(--text-2)', flexShrink:0}}>·</span>
      ...
      <span style={{color:'var(--text-3)', marginLeft:'auto', fontSize:'0.55rem', flexShrink:0}}>{tt('Mined blocks may be stale')}</span>
    </div>
  );
}
...
function HotMinerBanner({ tt = (x) => x, workers, aliases }) {
...
// ── Offline toast ─────────────────────────────────────────────────────────────
function OfflineToasts({ tt = (x) => x, workers, aliases }) {
  ...
  // which point we flash a green tt("✓ BACK ONLINE") confirmation that auto-fades
      ...
      {offlineCount > 0 && (
        <div style={{
          ...
          <div onClick={() => setCollapsed(c => !c)} style={{
            ...
            }} aria-label={tt("Dismiss all")} title={tt("Dismiss all")}>×</button>
          </div>
            ...
            <div style={{
              ...
              {offlineBanners.map(b => (
                <div key={b.name} style={{
                  ...
                  }} aria-label={tt("Dismiss this")}>×</button>
                </div>
              ))}
            </div>
          )}
...
function HashrateTrend({ tt = (x) => x, history, current }) {
  const trend = useMemo(() => {
...
// Renders a tt("Pool Stats") averages strip: one row per window
...
function StrikeVelocityChart({ tt = (x) => x, spsHistory, currentSps, hashrate, compact = false }) {
...
function HashrateChart({ tt = (x) => x, history, week, current, averages, compact = false }) {
  ...
  const inner = (
    <>
      ...
      <div style={{ fontFamily:'var(--fd)', fontSize:numberSize, fontWeight:700, letterSpacing:'0.01em', lineHeight:1, marginBottom:numberMarginBottom, display:'flex', alignItems:'baseline', flexWrap:'wrap', gap:'0.4rem', fontVariantNumeric:'tabular-nums' }}>
        ...
        <HashrateTrend tt={tt} history={history} current={current}/>
      </div>
...
// ── Worker grid ───────────────────────────────────────────────────────────────
function WorkerGrid({ tt = (x) => x, workers, aliases, onWorkerClick }) {
    ...
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      ...
      {sorted.length === 0 ? (
        <div style={{textAlign:'center',padding:'1.5rem',border:'1px dashed var(--border)',color:'var(--text-2)',fontSize:'0.75rem',fontFamily:'var(--fd)',lineHeight:2}}>
          {tt('No miners connected yet.')}<br/><span style={{fontFamily:'var(--fm)',fontSize:'0.7rem',color:'var(--cyan)'}}>stratum+tcp://umbrel.local:3333</span><br/><span style={{color:'var(--text-3)',fontSize:'0.65rem'}}>{tt('user: worker_name · pass: x')}</span>
        </div>
      ) : (
          ...
          {sorted.map(w=>{
            ...
            return(
              <div key={w.name} onClick={()=>onWorkerClick&&onWorkerClick(w)} style={{display:'flex',alignItems:'center',gap:'0.45rem',padding:'0.4rem 0.6rem',background:'var(--bg-raised)',border:`1px solid ${on?'rgba(57,255,106,0.12)':'transparent'}`,opacity:on?1:0.45,cursor:'pointer',transition:'background 0.15s', minWidth:0}}
                ...
                <span title={w.minerType||tt('Unknown')} style={{fontSize:11,color:on?'var(--cyan)':'var(--text-3)',width:12,textAlign:'center',flexShrink:0}}>{icon}</span>
...
function classifyShareTier(pctOfBlock, tt = (x) => x) {
  if (pctOfBlock >= 10)   return { label:tt('LEGENDARY'), color:'#ff5252', glow:true,  bgTint:'rgba(255,82,82,0.06)',  borderTint:'rgba(255,82,82,0.40)' };
  if (pctOfBlock >= 1)    return { label:tt('EPIC'),      color:'#ff8a3d', glow:true,  bgTint:'rgba(255,138,61,0.06)', borderTint:'rgba(255,138,61,0.35)' };
  if (pctOfBlock >= 0.1)  return { label:tt('RARE'),      color:'var(--amber)', glow:false, bgTint:'rgba(var(--amber-rgb),0.05)', borderTint:'rgba(var(--amber-rgb),0.25)' };
  if (pctOfBlock >= 0.01) return { label:tt('GOOD'),      color:'var(--cyan)',  glow:false, bgTint:'rgba(0,255,209,0.04)',  borderTint:'rgba(0,255,209,0.18)' };
  return                       { label:tt('NORMAL'),    color:'var(--text-2)', glow:false, bgTint:'transparent',         borderTint:'var(--border)' };
}
...
function ClosestCallsPanel({ tt = (x) => x, closestCalls, aliases, networkDifficulty }) {
  const list = closestCalls || [];
  if (!list.length) {
    return (
      <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
        <div style={{...cardTitle, color:'var(--amber)', flexShrink:0}}>{tt('▸ Near Strikes')}</div>
        <div style={{textAlign:'center',padding:'1.5rem',border:'1px dashed var(--border)',color:'var(--text-2)',fontSize:'0.72rem',fontFamily:'var(--fd)'}}>
          {tt('Building leaderboard…')}<br/>
          <span style={{color:'var(--amber)',fontSize:'0.65rem'}}>{tt('Shares tracked as they come in')}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      <div style={{...cardTitle, display:'flex', justifyContent:'space-between', alignItems:'center', color:'var(--amber)', flexShrink:0}}>
        <span>{tt('▸ Near Strikes')}</span>
        <span style={{color:'var(--amber)', fontFamily:'var(--fm)', fontSize:'0.6rem', letterSpacing:'0.08em', marginRight:'14px', whiteSpace:'nowrap'}}>{tt('fleet-wide')}</span>
      </div>
      <div style={{display:'flex', flexDirection:'column', gap:'0.35rem', flex:1, minHeight:0, overflowY:'auto'}}>
        {list.map((c, i) => {
          ...
          const tier = netDiff ? classifyShareTier(pctOfBlock, tt) : { label:'—', color:'var(--text-2)', glow:false, bgTint:'transparent', borderTint:'var(--border)' };
          return (
...
// ── Bitcoin Network ───────────────────────────────────────────────────────────
function NetworkStats({ tt = (x) => x, network, blockReward, mempool, prices, currency, privateMode, latestBlock }) {
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      <div style={{...cardTitle, color:'var(--amber)', flexShrink:0}}>{tt('▸ Bitcoin Network')}</div>
      {[[tt('Block Height'), fmtNum(network?.height), 'var(--text-1)'],
        [tt('Difficulty'), fmtDiff(network?.difficulty), 'var(--text-1)'],
        [tt('Net Hashrate'), fmtHr(network?.hashrate), 'var(--cyan)']].map(([l,v,c])=>(
      ...
      {blockReward && (
        <div style={{...statRow, background:'var(--bg-deep)', borderColor:'rgba(var(--amber-rgb),0.25)'}}>
          <span style={{...label, color:'var(--amber)'}}>{tt('🏆 Next Block Prize')}</span>
      ...
      {!privateMode && price!=null && (
        <div style={statRow}>
          <span style={label}>{tt('BTC Price')}</span>
      ...
      {mempool?.totalFeesBtc>0 && (
        <div style={statRow}>
          <span style={label}>{tt('Mempool Fees')}</span>
...
// ── Bitcoin Node panel ────────────────────────────────────────────────────────
function BitcoinNodePanel({ tt = (x) => x, nodeInfo }) {
  const ni = nodeInfo || {};
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      ...
      <div style={statRow}>
        <span style={label}>{tt('Client')}</span>
      ...
      <div style={statRow}>
        <span style={label}>{tt('Peers')}</span>
      ...
      <div style={statRow}><span style={label}>{tt('Relay Fee')}</span><span style={{fontFamily:'var(--fm)',fontSize:'0.85rem',color:'var(--amber)'}}>{relayStr}</span></div>
      <div style={statRow}><span style={label}>{tt('Mempool TXs')}</span><span style={{fontFamily:'var(--fm)',fontSize:'0.85rem',color:'var(--text-1)'}}>{fmtNum(ni.mempoolCount || 0)}</span></div>
      <div style={statRow}><span style={label}>{tt('Mempool Size')}</span><span style={{fontFamily:'var(--fm)',fontSize:'0.85rem',color:'var(--cyan)'}}>{fmtBytes(ni.mempoolBytes || 0)}</span></div>
      <div style={{flex:1,minHeight:0}}/>
    </div>
  );
}
...
// Replaces the old tt("Strike Odds") card. Fuses Strike Odds' orbital gauge with
...
function NonceField({ hashrate, huntAnim, performanceMode }) {
  ...
  // v1.11.41: Performance Mode ref — checked inside the rAF body to bail
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      // v1.11.41: Performance Mode short-circuit. Skip all draw work — the
  ...
  return (
    <div ref={containerRef} style={{
      ...
      {/* v1.11.41: Performance Mode static frame — overlays all canvases
      ...
      <div style={{display:'flex', flexDirection:'column', gap:'0.55rem', flex:1, minHeight:0}}>
        ...
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', gap:10}}>
            <div style={{display:'flex', alignItems:'baseline', gap:8, flex:1, minWidth:0}}>
              ...
              <span style={{
                ...
                {totalBtc > 0 ? totalBtc.toFixed(4) : '—'}<span style={{fontSize:'0.65rem', marginLeft:2, WebkitTextFillColor:'var(--amber-dim)'}}>{tt('BTC')}</span>
              </span>
            </div>
          ...
          {(subsidyBtc > 0 || feesBtc > 0) && (
            <div style={{display:'flex', gap:10, justifyContent:'center', alignItems:'baseline', marginTop:5, paddingTop:5, borderTop:'1px dashed rgba(var(--amber-rgb),0.18)', fontSize:'0.65rem'}}>
              <span style={{color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase'}}>{tt('Subsidy')}</span>
              <span style={{fontFamily:'var(--fm)', color:'var(--text-1)', fontWeight:600}}>{subsidyBtc.toFixed(3)}</span>
              <span style={{color:'var(--text-3)'}}>·</span>
              <span style={{color:'var(--text-2)', letterSpacing:'0.08em', textTransform:'uppercase'}}>{tt('Fees')}</span>
        ...
        {(feeFast || feeMid || feeLow) && (
          <div style={{display:'flex', gap:6, flexShrink:0}}>
            <div style={{flex:1, background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.4rem 0.35rem', textAlign:'center'}}>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--green)', textTransform:'uppercase'}}>{tt('⚡ FAST')}</div>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.85rem', color:'var(--text-1)', fontWeight:700, marginTop:3}}>
                {feeFast != null ? feeFast : '—'}
              </div>
              <div style={{fontFamily:'var(--fm)', fontSize:'0.55rem', color:'var(--text-2)', marginTop:1}}>{tt('sat/vB')}</div>
            </div>
            <div style={{flex:1, background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.4rem 0.35rem', textAlign:'center'}}>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--amber)', textTransform:'uppercase'}}>{tt('◐ MID')}</div>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.85rem', color:'var(--text-1)', fontWeight:700, marginTop:3}}>
                {feeMid != null ? feeMid : '—'}
              </div>
              <div style={{fontFamily:'var(--fm)', fontSize:'0.55rem', color:'var(--text-2)', marginTop:1}}>{tt('sat/vB')}</div>
            ...
            <div style={{flex:1, background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.4rem 0.35rem', textAlign:'center'}}>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.1em', color:'var(--text-2)', textTransform:'uppercase'}}>{tt('◯ LOW')}</div>
              <div style={{fontFamily:'var(--fd)', fontSize:'0.85rem', color:'var(--text-1)', fontWeight:700, marginTop:3}}>
                {feeLow != null ? feeLow : '—'}
              </div>
              <div style={{fontFamily:'var(--fm)', fontSize:'0.55rem', color:'var(--text-2)', marginTop:1}}>{tt('sat/vB')}</div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:4, flexShrink:0}}>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.35rem 0.3rem', textAlign:'center'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.58rem', letterSpacing:'0.08em', color:'var(--text-2)', textTransform:'uppercase'}}>{tt('Expected')}</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.72rem', color:'var(--amber)', fontWeight:700, marginTop:2}}>
              {fmtOdds(expectedDays)}
            </div>
          </div>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.35rem 0.3rem', textAlign:'center'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.58rem', letterSpacing:'0.08em', color:'var(--text-2)', textTransform:'uppercase'}}>{tt('Yearly')}</div>
          ...
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.35rem 0.3rem', textAlign:'center'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.58rem', letterSpacing:'0.08em', color:'var(--text-2)', textTransform:'uppercase'}}>{tt('Daily')}</div>
            <div style={{fontFamily:'var(--fm)', fontSize:'0.7rem', color:'var(--text-1)', fontWeight:700, marginTop:2}}>
              {perDay>0 ? fmtPct(perDay*100, 3) : '—'}
            </div>
          </div>
          <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.35rem 0.3rem', textAlign:'center'}}>
            <div style={{fontFamily:'var(--fd)', fontSize:'0.58rem', letterSpacing:'0.08em', color:'var(--text-2)', textTransform:'uppercase'}}>{tt('Sats/d')}</div>
...
const StratumPanel = React.memo(function StratumPanel_Impl({ tt = (x) => x, payoutAddress, stratumHealth, startedAt }) {
    ...
    return (
      ...
      >
        <span style={{fontSize:'0.6rem', letterSpacing:'0.05em', color:isCopied ? 'var(--green)' : (accent || 'var(--text-1)'), fontWeight:700}}>
          {ssl && <span style={{fontSize:'0.45rem', letterSpacing:'0.14em', opacity:0.85, marginRight:3}}>{tt('TLS')}</span>}{port}
        </span>
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      <div style={{...cardTitle, color:'var(--amber)', marginBottom:'0.5rem', flexShrink:0}}>{tt('▸ Stratum Connection')}</div>
      ...
      <div style={fieldRowStyle}>
        ...
        <div style={{...helperStyle, marginTop:5, fontSize:'0.6rem'}}>
          {tt('3333 ASIC · 3334 Hobby ·')} <span style={{display:'inline-block', padding:'1px 5px', borderRadius:3, fontSize:'0.55rem', letterSpacing:'0.14em', color:'var(--cyan)', border:'1px solid rgba(0,255,209,0.45)', background:'rgba(0,255,209,0.05)', verticalAlign:'1px', marginRight:4}}>{tt('TLS')}</span>{tt('4333 SSL')}
        </div>
      </div>
      ...
      <div style={fieldRowStyle}>
        ...
        {!payoutAddress && (
          <div style={{...helperStyle, color:'var(--amber)', marginTop:4}}>
            {tt('⚠ Set payout address in Settings')}
          </div>
        )}
      </div>
...
function PoolUptimeStrip({ tt = (x) => x, startedAt }) {
  ...
  return (
    <div style={{
      ...
      <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.5rem 0.4rem', textAlign:'center', minWidth:0, overflow:'hidden'}}>
        <div style={{fontFamily:'var(--fd)', fontSize:'0.5rem', letterSpacing:'0.13em', color:'var(--text-2)', textTransform:'uppercase', marginBottom:3}}>{tt('Uptime')}</div>
      ...
      <div style={{background:'var(--bg-raised)', border:'1px solid var(--border)', padding:'0.5rem 0.4rem', textAlign:'center', minWidth:0, overflow:'hidden'}}>
        <div style={{fontFamily:'var(--fd)', fontSize:'0.5rem', letterSpacing:'0.13em', color:'var(--text-2)', textTransform:'uppercase', marginBottom:3}}>{tt('Started')}</div>
...
// ── Hot Streak (luck) ─────────────────────────────────────────────────────────
function LuckGauge({ tt = (x) => x, luck }) {
  if (!luck) return null;
  ...
  return (
    <div style={{...card, display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      <div style={{...cardTitle, color:'var(--amber)', flexShrink:0}}>{tt('▸ Hot Streak')}</div>
      ...
      <div style={{...statRow, borderColor:'var(--border-hot, rgba(var(--amber-rgb),0.3))'}}>
        <span style={label}>{tt('Streak')}</span>
...
// ── Difficulty Retarget ───────────────────────────────────────────────────────
function RetargetPanel({ tt = (x) => x, retarget }) {
  ...
  return (
    <div style={{...card, minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      <div style={{...cardTitle, color:'var(--amber)', flexShrink:0}}>{tt('▸ Difficulty Retarget')}</div>
      <div style={{display:'flex',flexDirection:'column',gap:'0.5rem'}}>
        <div style={{textAlign:'center',padding:'0.25rem 0'}}>
          ...
          <div style={{fontFamily:'var(--fd)',fontSize:'0.55rem',letterSpacing:'0.15em',textTransform:'uppercase',color:'var(--text-2)',marginTop:4}}>{tt('estimated change')}</div>
          ...
          {prevDifficultyChange != null && (
            <div style={{fontFamily:'var(--fm)', fontSize:'0.72rem', color:'var(--text-2)', marginTop:6}}>
              {tt('Last epoch:')} <span style={{color:prevColor, fontWeight:600}}>{prevDifficultyChange>=0?'+':''}{prevDifficultyChange.toFixed(2)}%</span>
            </div>
          )}
        </div>
        <div>
          <div style={{display:'flex',justifyContent:'space-between',fontFamily:'var(--fd)',fontSize:'0.55rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--text-2)',marginBottom:4}}>
            <span>{tt('Epoch progress')}</span><span style={{color:'var(--cyan)'}}>{pct.toFixed(1)}%</span>
          </div>
        ...
        <div style={{...statRow,marginBottom:0}}><span style={label}>{tt('Remaining Blocks')}</span><span style={{fontFamily:'var(--fm)',fontSize:'0.85rem',color:'var(--text-1)'}}>{fmtNum(remainingBlocks)}</span></div>
        <div style={{...statRow,marginBottom:0}}><span style={label}>{tt('ETA')}</span><span style={{fontFamily:'var(--fm)',fontSize:'0.85rem',color:'var(--amber)'}}>{fmtDurationMs(remainingTime)}</span></div>
      </div>
      <div style={{flex:1,minHeight:0}}/>
    </div>
  );
...
function SetupForm({ tt = (x) => x, saveConfig }) {
  ...
  const submit = async () => {
    setErr('');
    if (!isValidBtcAddress(a)) { setErr(tt('Invalid BTC address')); return; }
    setLoading(true);
    try { await saveConfig({ payoutAddress: a.trim() }); } catch (e) { setErr(e.message || tt('Failed')); }
...
function HealthStatusCard({ tt = (x) => x, onOpen }) {
  ...
  const headlineMap = {
    green: { text: tt('ALL SYSTEMS GO'), color: 'var(--green)' },
    amber: { text: tt('MINOR ISSUES'),    color: 'var(--amber)' },
    red:   { text: tt('DEGRADED'),        color: 'var(--red)' },
  };
  ...
  const rows = [
    { key:'containers',  label:tt('CONTAINERS') },
    { key:'api',         label:tt('API') },
    { key:'persistence', label:tt('PERSISTENCE') },
    { key:'ckpool',      label:tt('CKPOOL') },
    { key:'zmq',         label:tt('ZMQ') },
    { key:'disk',        label:tt('DISK') },
  ];
...
function SettingsModal({ onClose, saveConfig, currentConfig, currency, onCurrencyChange, onResetLayout, workers, aliases, onAliasesChange, stripSettings, onStripSettingsChange, tickerSettings, onTickerSettingsChange, minimalMode, onMinimalModeChange, performanceMode, onPerformanceModeChange, desktopCardMode, onDesktopCardModeChange, isMobileView = false, lang = 'en', onLangChange, visibleCards, onVisibleCardsChange, networkStats, onNetworkStatsRefresh, carouselEnabled, onCarouselChange, pulseAnim, onPulseAnimChange, huntAnim, onHuntAnimChange, onPreviewCelebration, poolPin, onPoolPinChange, debugSettings, onDebugSettingsChange, themeId, onThemeChange }) {
  ...
  // where tt('SoloStrike') is now hardcoded server-side. No user-facing effect lost.
...
// ── v1.11.41 ── Static Pulse Strikes (Performance Mode overlay) ─────────────
...
// ── v1.11.41 ── Static Pulse Strikes — DOM divs ────────────────────────────
...
const PulsePanel = React.memo(function PulsePanel_Impl({ networkStats, onOpenSettings, onOpenStrikers, pulseAnim = 'block', performanceMode = false, compact = false, poolPin = null, onPoolPinChange = null, lastShareAt = null, acceptedCount = 0, workers = null, lang = 'en', showStamp = true }) {
  ...
  // v1.11.41 — Pinch-zoom + pan state for Performance Mode static
  ...
  // v1.11.x: Strike Mesh Simulator. Tap tt("◈ Simulate") in the top-right
  ...
  // v1.11.41: Performance Mode ref — same pattern as NonceField. Live
  ...
  const handleCanvasTap = useCallback((e) => {
    ...
    // v1.11.41: Performance Mode uses a flat equirectangular map (not a
    ...
    if (perfModeRef.current) {
      // v1.11.41: Suppress tap-to-pin if the user just finished a pinch
      // gesture (didPinchRef gets set on 2-finger touchend).
      if (didPinchRef.current) {
        didPinchRef.current = false;
        return;
      }
      // v1.11.41: Inverse-transform click coords if user has zoomed/panned.
      ...
      // v1.11.41: Standard equirectangular [-90, +90] — keeps pin
  ...
  // v1.11.41 — Pinch-zoom + pan touch handlers for the Performance Mode
  ...
  useEffect(() => {
    ...
    // v1.11.41: Click → place pin in Performance Mode (when pulseAnim is
  ...
  useEffect(() => {
    ...
    const drawGlobe = (dt, W, H) => {
      ...
      for (const p of pools) {
        ...
        if (p.isOwn) {
          ...
          // so the tt("I am you") identity is always visible.
    ...
    const draw = (now) => {
      ...
      // v1.11.41: Performance Mode short-circuit. The static <img> overlay
  ...
  return (
    <>
      ...
      <div style={{position:'relative'}}>
        ...
        <div ref={containerRef} style={{
          width:'100%',
          // v1.11.41: Compact layout — mirror the live globe sizing.
          ...
          />
          {/* v1.11.41: Performance Mode static Pulse map (compact layout). */}
          {/* v1.11.41 — Pinch-zoom wrapper (same as full layout below). */}
          {performanceMode && (
            <div
          ...
          {/* v1.11.41: DOM strike markers — CSS-keyframe animated rings
            ...
            <>
              <div
                ...
                style={{
                  ...
                  color: 'var(--amber)', fontSize: '0.55rem',
                  letterSpacing: '0.16em', cursor: 'pointer',
                  userSelect: 'none', fontWeight: 600, zIndex: 5,
                  textShadow: '0 0 8px rgba(var(--amber-rgb),0.5)',
                }}
                role="button"
                aria-label={tt("Open block constellation simulator")}
              >{tt('◈ Simulate Mesh')}</div>
        ...
        {/* Footer tagline — single line in compact, leave room for stamp on right.
            v1.8.8-globe-rev7: when tt('Solo Strike Map') is the active animation we
        ...
        <div
          ...
          {pulseAnim === 'globe' ? (
            <span style={{fontStyle:'italic', letterSpacing:'0.04em'}}>
              {tt('Pool locations are approximate.')}
            </span>
          ) : (
            <>
              <span style={{color:'var(--amber)', fontWeight:600}}>{tt('100% SOLO ·')}</span> {tt('Your blocks stay yours.')}
            </>
          )}
          {onOpenStrikers && (ns.peers && ns.peers.length > 0) && (
            <span style={{marginLeft:6, color:'var(--amber)', fontFamily:'var(--fd)', fontSize:'0.5rem', letterSpacing:'0.12em'}}>{tt('▸ TAP STRIKERS')}</span>
          )}
        </div>
        </div>
        <StampSolo/>
    ...
    <div style={{...card, position:'relative', minWidth:0, maxWidth:'100%', overflow:'hidden', display:'flex', flexDirection:'column', height:'100%'}} className="fade-in ss-card-chrome">
      ...
      <div
        style={{
          // v1.11.41: Restored flex:1 always — the inner containerRef now
      ...
      <div ref={containerRef} style={{
        width:'100%',
        // v1.11.41: Mirror the LIVE GLOBE sizing exactly — flex:1 with
        ...
        />
        {/* v1.11.41: Performance Mode static Pulse map. Overlays globe and
            ...
            pin placement. */}
        {/* v1.11.41 — Pinch-zoom + pan wrapper. Map, mesh, and SVG strikes
          ...
          <>
            <div
              ...
              style={{
                ...
                color: 'var(--amber)', fontSize: '0.6rem',
                letterSpacing: '0.18em', cursor: 'pointer',
                userSelect: 'none', fontWeight: 600, zIndex: 5,
                textShadow: '0 0 8px rgba(var(--amber-rgb),0.5)',
              }}
      ...
      {/* Footer tagline.
          ...
          tt('Solo Strike Map') is the active animation. Other animations unchanged.
      ...
      <div
        ...
        {pulseAnim === 'globe' ? (
          ...
          <>
            Pulse is a census, not a pool. <span style={{color:'var(--amber)', fontWeight:600}}>{tt('Your blocks stay 100% yours.')}</span>
          </>
        )}
        {onOpenStrikers && (ns.peers && ns.peers.length > 0) && (
          <span style={{display:'block', marginTop:4, color:'var(--amber)', fontFamily:'var(--fd)', fontSize:'0.55rem', letterSpacing:'0.12em'}}>{tt('▸ TAP TO SEE STRIKERS')}</span>
        )}
      </div>
      </div>
      <StampSolo/>
...
//
// Section names (tt("FIREPOWER — LIVE") and tt("SOLOSTRIKE PULSE")) are preserved
...
function StrikersModal({ tt = (x) => x, networkStats, onClose }) {
  ...
  // Small inline helper for ATH timestamp (tt("Peak X · 3d ago"))
  ...
  const flagFromLoc = (loc) => {
    ...
    if (lat >= 25 && lat <= 50 && lon >= -125 && lon <= -65) return { flag: '🇺🇸', label: tt('US') };
    if (lat >= 45 && lat <= 70 && lon >= -140 && lon <= -55) return { flag: '🇨🇦', label: tt('CA') };
    if (lat >= -35 && lat <= 15 && lon >= -85 && lon <= -35)  return { flag: '🇧🇷', label: tt('SA') };
    if (lat >= 35 && lat <= 70 && lon >= -10 && lon <= 30)    return { flag: '🇪🇺', label: tt('EU') };
    if (lat >= 50 && lat <= 60 && lon >= -10 && lon <= 5)     return { flag: '🇬🇧', label: tt('UK') };
    if (lat >= 30 && lat <= 50 && lon >= 125 && lon <= 150)   return { flag: '🇯🇵', label: tt('JP') };
    if (lat >= 15 && lat <= 55 && lon >= 70 && lon <= 135)    return { flag: '🌏', label: 'Asia' };
    if (lat >= -45 && lat <= -10 && lon >= 110 && lon <= 180) return { flag: '🇦🇺', label: tt('AU') };
  ...
  const deriveBadges = (p, allPeersSorted) => {
    ...
    if (ageDays >= 90) badges.push(tt('OG'));
    if (ageDays < 7 && ageDays >= 0) badges.push(tt('NEW'));
    ...
    if (myRank >= 0 && myRank < whaleCutoff && p.hashrate > 0) badges.push(tt('WHALE'));
    ...
    if (Array.isArray(p.loc) && p.loc.length === 2) {
      ...
      if (!isCommon) badges.push(tt('GLOBETROTTER'));
    }
  ...
  // v1.12.x: tt("Joined Nd ago") formatter. Takes unix seconds, returns short
  ...
  // (and yourself). Each row shows the Striker label + block height + tt("Xd ago").
...
// tt("The Reckoning") — drill-down from the Strike Odds card. Lets the user
    ...
    try {
      const bundle = {
        ...
        streams: {
          ...
          // v1.11.41 — stutter diagnosis streams
...
function PoolAlignmentBlock({ tt = (x) => x, worker }) {
  ...
  const recheck = async () => {
    ...
    try {
      ...
      if (!r.ok || !data.ok) throw new Error(data.error || tt('Recheck failed'));
      ...
      setError(e.message || tt('Recheck failed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div style={section}>
      ...
      {(pa.status === 'aligned' || pa.status === 'backup' || pa.status === 'misaligned' || pa.status === 'unverifiable') && pa.activePool && (
        <div style={kvRow}>
          <span style={kvLabel}>{tt('Active pool')}</span>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(6,7,8,0.88)',backdropFilter:'blur(4px)',WebkitBackdropFilter:'blur(4px)',display:'flex',alignItems:'flex-start',justifyContent:'center',zIndex:250,padding:'calc(env(safe-area-inset-top) + 1rem) 0.75rem 0.75rem',overflowY:'auto'}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{width:'100%',maxWidth:560,background:'var(--bg-surface)',border:'1px solid var(--border-hot)',boxShadow:'var(--glow-a)',maxHeight:'calc(100dvh - 4rem)',overflowY:'auto'}}>
        <div style={{padding:'1rem 1.25rem',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'0.75rem'}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontFamily:'var(--fd)',fontSize:'0.58rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--text-2)',marginBottom:6}}>
              {w.minerType || tt('Unknown miner')}{w.minerVendor && ` · ${w.minerVendor}`}
            </div>
            <div style={{display:'inline-flex',alignItems:'center',gap:5,fontFamily:'var(--fd)',fontSize:'0.58rem',letterSpacing:'0.12em',textTransform:'uppercase'}}>
              ...
              <span style={{color:on?'var(--green)':'var(--red)'}}>{on?'ONLINE':tt('OFFLINE')}</span>
        ...
        <div style={{padding:'1rem 1.25rem'}}>
          ...
          <PoolAlignmentBlock tt={tt} worker={w}/>
...
export default function App() {
  ...
  // Build ticker text
  // v1.11.41: tickerText is recomputed on every poolState change, but the
  ...
  // v1.11.41: pillsSource() returns one fresh metric per call (round-robin).
  ...
  if (poolState && !poolState.payoutAddress) {
    return (
      <>
        <Header tt={tt} connected={connected} status="setup" onSettings={()=>setShowSettings(true)} privateMode={!!poolState.privateMode} minimalMode={minimalMode} performanceMode={performanceMode} zmq={poolState?.zmq}/>
  ...
  const cardComponents = {
    ...
    />,
    strikevel: <StrikeVelocityChart tt={tt}
    ...
    pulse: <PulsePanel
      ...
      showStamp={false}
    />,
    workers: <WorkerGrid tt={tt} workers={workers} aliases={aliases} onWorkerClick={setSelectedWorker}/>,
    network: <NetworkStats tt={tt} network={poolState?.network} blockReward={poolState?.blockReward} mempool={poolState?.mempool} prices={poolState?.prices} currency={currency} privateMode={!!poolState?.privateMode} latestBlock={poolState?.latestBlock}/>,
    node: <BitcoinNodePanel tt={tt} nodeInfo={poolState?.nodeInfo}/>,
    stratum: <StratumPanel tt={tt} payoutAddress={poolState?.payoutAddress} stratumHealth={stratumHealth} startedAt={poolState?.shareStatsStartedAt}/>,
    ...
    luck: <LuckGauge tt={tt} luck={poolState?.luck}/>,
    retarget: <RetargetPanel tt={tt} retarget={poolState?.retarget}/>,
    ...
    connstates:  <ConnectionStates tt={tt} pool={poolState?.pool} />,
     ...
     <div ref={headerRef} className="ss-app-header" style={{ position:'sticky', top:0, zIndex:50, background:'rgba(6,7,8,0.92)', backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', width:'100%', maxWidth:'100%', boxSizing:'border-box', overflow:'hidden', paddingTop:'env(safe-area-inset-top)' }}>
        ...
        {showChrome && !minimalMode && (
          <>
            ...
            <SyncWarningBanner tt={tt} sync={poolState?.sync}/>
          </>
        )}
      </div>
        ...
        {useCarousel ? (
          ...
          <DesktopPages
            cardComponents={Object.fromEntries(Object.keys(cardComponents).map(id => [id,
              <ErrorBoundary tt={tt} label={id}>{cardComponents[id]}</ErrorBoundary>
            ]))}
      ...
      <OfflineToasts tt={tt} workers={workers} aliases={aliases}/>
      <HotMinerBanner tt={tt} workers={workers} aliases={aliases}/>
      {selectedWorker && (
