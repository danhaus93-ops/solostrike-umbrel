...
async function writeCkpoolConf(address) {
  const conf = {
    btcd: [{ url: `http://${RPC_HOST}:${RPC_PORT}`, auth: `${RPC_USER}:${RPC_PASS}`, notify: true }],
    btcaddress: address,
    btcsig: 'SoloStrike/',
    blockpoll: 100,
    nonce1length: 4,
...
function parseLine(line) {
  ...
  if (RE_SHARE.test(line)) {
    ...
    if (!state.workers[key]) state.workers[key] = { name: key, hashrate: 0, shares: 0, rejected: 0, lastSeen: Date.now(), diff: 0, status: 'online' };
    state.workers[key].lastSeen = Date.now();
    state.workers[key].diff = parseFloat(diff);
    state.workers[key].status = 'online';
    if (status === 'accepted') { state.shares.accepted++; state.workers[key].shares++; }
    else if (status === 'rejected') { state.shares.rejected++; state.workers[key].rejected++; }
    else if (status === 'stale') state.shares.stale++;
    return;
  }
  if (RE_WORKER.test(line)) {
    const m = line.match(RE_WORKER);
    const name = m[1];
    if (!state.workers[name]) state.workers[name] = { name, hashrate: 0, shares: 0, rejected: 0, lastSeen: Date.now(), diff: 0, status: 'online' };
...
setInterval(() => {
  ...
  Object.values(state.workers).forEach(w => { w.status = w.lastSeen < cutoff ? 'offline' : 'online'; });
}, 30000);
...
function publicState() {
  ...
  const totalHr = workers.filter(w => w.status !== 'offline').reduce((s, w) => s + (w.hashrate || 0), 0);
  const netHr = state.network.hashrate || 1;
  const perBlock = totalHr > 0 ? totalHr / netHr : 0;
  return {
    config: { poolName: cfg.poolName, hasAddress: !!cfg.payoutAddress },
    status: state.status,
    hashrate: { current: totalHr, history: state.hashrate.history },
    workers,
    shares: state.shares,
    blocks: state.blocks,
    network: state.network,
    odds: { perBlock, expectedDays: perBlock > 0 ? 1 / (perBlock * 144) : null },
    uptime: state.uptime,
  };
}
...
app.get('/api/config', (_, res) => res.json({
  ...
  addressMasked: cfg.payoutAddress ? `${cfg.payoutAddress.slice(0,8)}...${cfg.payoutAddress.slice(-6)}` : null,
...
app.post('/api/config', async (req, res) => {
  ...
  if (poolName) cfg.poolName = poolName.slice(0, 32);
  await saveCfg();
  state.status = cfg.payoutAddress ? 'running' : 'no_address';
  broadcast({ type: 'CONFIG_UPDATED', data: { hasAddress: !!cfg.payoutAddress, poolName: cfg.poolName } });
  res.json({ ok: true });
});
...
async function boot() {
  await loadCfg();
  if (cfg.payoutAddress) { await writeCkpoolConf(cfg.payoutAddress); state.status = 'running'; }
  else state.status = 'no_address';
  watchLogs();
  await pollNetwork();
  server.listen(3001, () => console.log('[SoloStrike API] Listening on :3001'));
}
boot();
