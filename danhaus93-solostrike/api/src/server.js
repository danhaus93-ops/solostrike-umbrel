...
// Default to mempool.space public API. Users who install the Umbrel Mempool app
// can override via MEMPOOL_API_URL in docker-compose.yml to keep data local.
...
console.log(`[SoloStrike API] mempool source: ${MEMPOOL_API_URL}`);
...
async function pollMempool() {
  ...
  try {
    const [feesRes, mempoolRes, diffRes, blocksRes] = await Promise.all([
      fetchWithTimeout(`${MEMPOOL_API_URL}/v1/fees/recommended`,     { timeout: 6000 }).catch(() => null),
      fetchWithTimeout(`${MEMPOOL_API_URL}/mempool`,                  { timeout: 6000 }).catch(() => null),
      fetchWithTimeout(`${MEMPOOL_API_URL}/v1/difficulty-adjustment`, { timeout: 6000 }).catch(() => null),
      fetchWithTimeout(`${MEMPOOL_API_URL}/v1/blocks`,                { timeout: 8000 }).catch(() => null),
    ]);
