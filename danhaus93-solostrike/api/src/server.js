...
// Umbrel home-screen widget endpoint (v1.7.7) — returns the four-stats JSON
// shape Umbrel expects. Mounted on /api/widget/four-stats; declared in
// umbrel-app.yml widgets[0].endpoint and routed via app_proxy extras.
app.get('/api/widget/four-stats', (req, res) => {
  // Inline hashrate formatter — keeps widget self-contained
  const hr = state.hashrate?.current || 0;
  const fmtWidgetHr = (h) => {
    if (!h || h <= 0) return { text: '0', sub: 'H/s' };
    const units = [
      { d: 1e18, s: 'EH/s' },
      { d: 1e15, s: 'PH/s' },
      { d: 1e12, s: 'TH/s' },
    ...
    for (const u of units) {
      if (h >= u.d) {
        const v = h / u.d;
        return { text: v < 10 ? v.toFixed(2) : v < 100 ? v.toFixed(1) : Math.round(v).toString(), sub: u.s };
      }
    }
    return { text: Math.round(h).toString(), sub: 'H/s' };
  };
  // Best-share formatter — same style as the dashboard
  const fmtWidgetDiff = (d) => {
    if (!d || d <= 0) return '0';
  ...
  const hrFmt = fmtWidgetHr(hr);
  const workers = state.workers || {};
  const workersOnline = Object.values(workers).filter(w => w && w.status !== 'offline').length;
  const blocksFound = Array.isArray(state.blocks) ? state.blocks.length : 0;
  const bestDiff = fmtWidgetDiff(state.bestshare || 0);

  res.json({
    type: 'four-stats',
    link: '',
    items: [
      { title: 'Pool Hashrate', text: hrFmt.text,             subtext: hrFmt.sub },
      { title: 'Workers',       text: String(workersOnline),  subtext: '' },
      { title: 'Blocks Found',  text: String(blocksFound),    subtext: '' },
      { title: 'Best Diff',     text: bestDiff,               subtext: '' },
    ],
  });
});
