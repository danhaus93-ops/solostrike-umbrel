...
// Node 18+ ships fetch natively as a global — no external dep needed.
// Wrap it to support the { timeout } option we used to get from node-fetch v2.
...
const app = express();
app.use(cors());
app.use(express.json({ limit: '16kb' })); // tight limit — our endpoints take tiny payloads
...
// --- Solo / community pool detection for recent-blocks feed -------------------
...
// --- Block detection from ckpool log -----------------------------------------
...
function watchLogs() {
  ...
  const read = async () => {
    try {
      ...
      console.error('[LogWatch]', e.message);
    }
  };
...
// WebSocket keepalive — prevents idle connection teardown by any intermediary.
...
app.get('/api/health', (req, res) => res.json({ ok: true, uptime: Date.now() - state.uptime, version: '1.2.1' }));
...
app.get('/api/export/workers.csv', rateLimit, (req, res) => {
  ...
  const rows = [
    ...
    ['# solostrike_version', '1.2.1'],
    ['workername','miner_type','hashrate_hps','shares','rejected','bestshare','difficulty','status','health','last_seen_iso'],
  ];
  (s.workers || []).forEach(w => {
    rows.push([
...
async function boot() {
  ...
  const PORT = 3001;
  server.listen(PORT, () => console.log(`[SoloStrike API v1.2.1] Listening on :${PORT}`));
}
